...
#There are defifinetly differences in the relationships, with the most obvious possibly being amorphophallus's relationship to saccharum, aechmea, and neomarica. The length of the branches are much longer between the common ancestor and all taxa in R20120829. Also, distance between the individual taxa differ by tree. 
...
The OTL API provides services to access to the Tree of Life, which uses a system for naming and identifying taxa called OTT (for Open Tree Taxonomy). If you have names of species (or genera/families) then you can query for the OTT directly using features of ROTL, as below.
...
illicium_tree <- tol_subtree(ott_id(illicium_id))
