...
tr_primates
# we are creating a phylogenetic tree with 5 tips and 4 internal nodes, tips labels "humans" "chimps" "gorillas" "orangs" "lemur" (i added that one in on accident haha)
...
str(tr_primates)
#List of 3
 $ edge     : int [1:8, 1:2] 6 7 8 8 7 6 1 1 7 8 ...
 $ Nnode    : int 4
 $ tip.label: chr [1:5] "Humans" "Chimps" "Gorillas" "Orangs" ...
 - attr(*, "class")= chr "phylo"
 - attr(*, "order")= chr "cladewise"
attributes(tr_primates)

names(tr_primates) 
#"edge" "Nnode" "tip.label"-> the names of the different parts of the tree
...
```{r}
newick_str = "(((Burgers, Fries), Pizza), Soda);"
tr_food = read.tree(text=newick_str)
plot(tr_food)
```
#not sure if we were supposed to use plants, but I used food 'cause I was feeling peckish 
...
plot(tr2_plants)
```
# this is a much better tree, seeing as cabbage and kale are a lot closer than corn and cabbage (as depicted in the former tree)
...
#here is the fake tree I created 
...
#here is the correct phylogeny (I did my best)
```{r}
newick_str = "(((Cabbage, Radish, Kale), Wheat, Corn) Seqouia);"
trcorrect_plants = read.tree(text=newick_str)
plot(trcorrect_plants)
```
#I did my best, but I am not sure why Sequoia didn't plot 
...
# the branches are longer/shorter (to create a representation of time)
...
```{r}
layout(matrix(1:4, 2, 2))
plot(trcorrect_plants, type="cladogram")
plot(trcorrect_plants, type="fan")
plot(trcorrect_plants, type="unrooted")
plot(trcorrect_plants, type="radial")
```
#here are four types of the correct phylogeny (again, not sure why sequiao didn't plot)-> also not sure why the trees are cut off a little 
...
#four types of trees for the imported list: 
```{r}
layout(matrix(1:4, 2, 2))
plot(tree_phylomatic_EG2, type="cladogram")
plot(tree_phylomatic_EG2, type="fan")
plot(tree_phylomatic_EG2, type="unrooted")
plot(tree_phylomatic_EG2, type="radial")
```
...
#A few diferent versions of the stored tree:  
```{r}
layout(matrix(1:4, 2, 2))
plot(tree_phylomatic, type="cladogram")
plot(tree_phylomatic, type="fan")
plot(tree_phylomatic, type="unrooted")
plot(tree_phylomatic, type="radial")
```
#Really confused on what the question is asking. I see very little difference between the generated trees and the stored tree, other than layout of course. 
...
#For the imported list, a few of those taxa missing are from other kingdoms (algae, fungus, etc.). Botany has changed to not include those taxa within the greater plant phylogeny-> they have their own phylogeny. Some are mosses, ferns, etc., that may be more distantly related. However I am not sure why some, like pinus, are not included? 
...
#here is a phylogeny from my Mannahatta list :) 
```{r}
EG3<-c("Acer",
"Carpinus",
"Eurybia",
...
"Athyrium")
EG3
tree_phylomatic_EG4 <- phylomatic(taxa = EG3, get = 'POST')
plot(tree_phylomatic_EG4, no.margin = TRUE, type="cladogram")
newick_fn5 = "tree_phylomatic_EG4"
write.tree(tree_phylomatic_EG4, file=newick_fn5)
#repeating to get tree from Zanne2014
tree_phylomatic_EG5 <- phylomatic(taxa = EG3, storedtree ="zanne2014", get = 'POST')
plot(tree_phylomatic_EG5, no.margin = TRUE, type="cladogram")
newick_fn6 = "tree_phylomatic_EG5"
write.tree(tree_phylomatic_EG5, file=newick_fn6)
```
#comparison of the different types
```{r}
layout(matrix(1:4, 2, 2))
plot(tree_phylomatic_EG5, type="cladogram")
plot(tree_phylomatic_EG5, type="fan")
plot(tree_phylomatic_EG5, type="unrooted")
plot(tree_phylomatic_EG5, type="radial")
```
```{r}
newick_fn6 = "tree_phylomatic_EG5"
write.tree(tree_phylomatic_EG5, file=tree_phylomatic_EG5)
```

```{r}
pdf_fn = "tree_phylomatic_EG5.pdf"
pdf(file=pdf_fn)
plot(tree_phylomatic_EG5, type="unrooted", edge.width=5, edge.color="blue", lab4ut="axial")
dev.off()
```

#here is the phylogeny for my Plant-O-Matic list: 
```{r}
EG4<-c("Acer",
"Acorus",
"Adiantum",
...
"Zygodon")
EG4
tree_phylomatic_EG6 <- phylomatic(taxa = EG4, get = 'POST')
plot(tree_phylomatic_EG6, no.margin = TRUE, type="cladogram")
newick_fn7 = "tree_phylomatic_EG6"
write.tree(tree_phylomatic_EG6, file=newick_fn7)
#repeating to get tree from Zanne2014
tree_phylomatic_EG7 <- phylomatic(taxa = EG4, storedtree ="zanne2014", get = 'POST')
plot(tree_phylomatic_EG7, no.margin = TRUE, type="cladogram")
newick_fn8 = "tree_phylomatic_EG7"
write.tree(tree_phylomatic_EG7, file=newick_fn8)
```
#comparison of the different types
```{r}
layout(matrix(1:4, 2, 2))
plot(tree_phylomatic_EG7, type="cladogram")
plot(tree_phylomatic_EG7, type="fan")
plot(tree_phylomatic_EG7, type="unrooted")
plot(tree_phylomatic_EG7, type="radial")
```
